...
@section('content')
    <div class="container-fluid">
        <div class="mb-4">
            <h3 class="fw-bold" style="color: #f97316;">Ubah Data Monitoring</h3>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb" style="font-size: 12px;">
                    <li class="breadcrumb-item"><a href="{{ route('monitorings.index') }}" class="text-decoration-none">Data
                            Monitoring</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Ubah Monitoring</li>
                </ol>
            </nav>
        </div>
                    ...
                    <select name="student_id"
                        ...
                        @foreach ($students as $student)
                            <option value="{{ $student->id }}"
                                {{ old('student_id', $monitoring->student_id) == $student->id ? 'selected' : '' }}>
                    ...
                    <select name="company_id"
                        ...
                        @foreach ($companies as $company)
                            <option value="{{ $company->id }}"
                                {{ old('company_id', $monitoring->company_id) == $company->id ? 'selected' : '' }}>
                                {{ $company->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="teacher_id"
                        ...
                        @foreach ($teachers as $teacher)
                            <option value="{{ $teacher->id }}"
                                {{ old('teacher_id', $monitoring->teacher_id) == $teacher->id ? 'selected' : '' }}>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="start_date"
                            ...
                            value="{{ old('start_date', $monitoring->start_date) }}" required>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="end_date"
                            ...
                            value="{{ old('end_date', $monitoring->end_date) }}" required>
                    </div>
                </div>
